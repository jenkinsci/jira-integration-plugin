		...
		extends ManagementLink
{
	...
	{
		return sitesConfiguration.stream().sorted(comparing(JiraSite::getName)).collect(toCollection(LinkedHashSet::new));
	}
	...
	public String getSiteUrl(String url)
	{
		...
		return sitesConfiguration.stream().filter(site -> {
			try
			{
				return site.getUri().equals(uri);
			}
			catch (Exception e)
			{
			...
			throws IOException
	{
		...
		{
			JiraSitesConfiguration.get().registerSite(jiraSite);
		}
			...
			throws IOException
	{
		...
		JiraSitesConfiguration.get().unregisterSite(URI.create(getJsonFromRequest(request).getString("url")));
	}
}
